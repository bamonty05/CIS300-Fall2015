/* DirectedGraph.cs
 * Author: Rod Howell and J-Dizzle
 * Code: 86 51 11 47
 */
using System;
...
namespace Ksu.Cis300.ShortestPaths
{
    ...
    public class DirectedGraph<TNode, TEdge>
    {
        ...
        public void AddEdge(TNode source, TNode dest, TEdge value)
        {
            ...
            {
                throw new ArgumentNullException("Both are null");
            }
            else if (source.Equals(dest))
            {
                throw new ArgumentNullException("Equal each other");
            }
            else if (_edges.ContainsKey(new Tuple<TNode, TNode>(source, dest)))
            {
                throw new ArgumentNullException("Already contains value");
            }
            Tuple<TNode, TNode> newEdge = new Tuple<TNode, TNode>(source, dest);
            _edges.Add(newEdge, value);

            LinkedListCell<Tuple<TNode, TEdge>> temp = new LinkedListCell<Tuple<TNode, TEdge>>();
            temp.Data = new Tuple<TNode, TEdge>(dest, value);

            LinkedListCell<Tuple<TNode, TEdge>> cell;
            if (!_adjacencyLists.TryGetValue(source, out cell))
            {
                _adjacencyLists.Add(source, temp);
            }
            else
            {
                temp.Next = cell;
                _adjacencyLists[source] = temp;
            }
